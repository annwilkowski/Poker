...
module poker_hand_tb;
    ...
    logic winner;
    logic is_draw;
    logic show_wait_screen;
    logic show_game_screen;
    ...
    logic [MAX_STACK_W-1:0] player1_invested;
    logic [MAX_STACK_W-1:0] player2_invested;
    always #5 clk = ~clk;
    ...
    initial begin
        ...
        wait (show_wait_screen);
        bet_input = 11'd13;
        toggle(advance_button);
        hold(bet_or_raise_button);
        wait (show_wait_screen);
        ...
        while (curr_round_state != showdown) begin
            toggle(advance_button);
            toggle(check_or_call_button);
            wait (show_wait_screen || (curr_round_state == showdown));
        end
        $display("round done");
    ...
    endtask
        ...
        task automatic hold(ref logic signal);
        @(posedge clk);
        signal = 1;
