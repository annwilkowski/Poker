...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	u8 Send_Buf[bufsize];
	Send_Buf[0] = reg + 2; //reg + 2 is the command byte
	for (int i = 1; i < bufsize; i++) {
		Send_Buf[i] = data[i - 1]; //data is the data to be written
	}

	int status = XSpi_Transfer(&SpiInstance, Send_Buf, NULL, bufsize);
	if (status != XST_SUCCESS) {
		xil_printf("SPI MAXbytes_wr transfer failed %d", status);
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//read val via SPI
	//read return code from SPI peripheral 
	int status = XSpi_Transfer(&SpiInstance, Send_Buf, Recv_Buf, 2);
	//if return code != 0 print an error
	if (status != XST_SUCCESS) {
		xil_printf("SPI MAXreg_rd transfer failed %d", status);
	}
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	//return val
	return Recv_Buf[1]; //return the value read from the register
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	u8 Send_Buf[bufsize];
	u8 Recv_Buf[bufsize];
	Send_Buf[0] = reg; //reg is the command byte
	for (int i = 1; i < bufsize; i++) {
		Send_Buf[i] = 0; //initialize the buffer to zero
	}

	int status = XSpi_Transfer(&SpiInstance, Send_Buf, Recv_Buf, bufsize);
	//if return code != 0 print an error
	if (status != XST_SUCCESS) {
		xil_printf("SPI MAXbytes_rd transfer failed %d", status);
	}
	//copy the data from Recv_Buf to data
	for (int i = 0; i < nbytes; i++) {
		data[i] = Recv_Buf[i + 1]; //copy the data from the buffer
	}
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	return (data + nbytes);
