...
#define COLUMNS 80
#define ROWS 30
#define VRAM_SIZE      (ROWS * COLUMNS * 2)  // 4800 bytes (30x80x2)
#define PALETTE_OFFSET  0x2000               // Palette starts at 0x2000
#define PADDING_SIZE    (PALETTE_OFFSET - VRAM_SIZE) // 8192 - 4800 = 3392 bytes

struct TEXT_HDMI_STRUCT {
	uint8_t		      	VRAM [VRAM_SIZE]; 						 //Week 2 - extended VRAM
	//modify this by adding const bytes to skip to palette, or manually compute palette
	uint8_t				reserved [PADDING_SIZE];
	uint8_t				Palette [32];  // Anna added
};
